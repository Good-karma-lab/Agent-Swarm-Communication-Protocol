...
1. `$OPENSWARM_IDENTITY_PATH` (env override)
...
| `openswarm-connector/src/config.rs` | Add `IdentityConfig` section |
| `openswarm-connector/src/main.rs` | Call `load_or_create_keypair` before building swarm |
...
```bash
openswarm-connector --bootstrap-mode --listen /ip4/0.0.0.0/tcp/9000
```
...
| `openswarm-connector/src/config.rs` | Add `bootstrap_mode: bool` to `NetworkConfig` |
| `openswarm-connector/src/main.rs` | Add `--bootstrap-mode` CLI flag; apply bootstrap behavior |
...
| `openswarm-connector/src/config.rs` | Add `auto_discover_bootstrap: bool` (default: true) |
...
| `openswarm-connector/src/config.rs` | Add NAT traversal flags to `NetworkConfig` |
...
**DHT key:** `/openswarm/names/sha256(<lowercase(name)>)`
...
resolve("alice")
  ...
  → Kademlia GET /openswarm/names/<key>
...
| `openswarm-connector/src/rpc_server.rs` | Add 5 new RPC methods above |
| `openswarm-connector/src/config.rs` | Add `wws_name: Option<String>` to `AgentConfig` |
...
| File | Change |
|------|--------|
| `openswarm-connector/src/rpc_server.rs` | Add signature verification middleware |
...
| New: `openswarm-connector/src/auth.rs` | Token bucket rate limiter, tier ACL checks |
...
3. Places it at `~/.local/bin/openswarm-connector`
...
```dockerfile
FROM scratch
COPY openswarm-connector /openswarm-connector
ENTRYPOINT ["/openswarm-connector"]
```
...
docker run -d \
  -v ~/.openswarm:/data \
  -e OPENSWARM_IDENTITY_PATH=/data/identity.key \
  -e OPENSWARM_AGENT_NAME=my-agent \
  -p 9000:9000 \
