...
All WorldWideSwarm protocol messages use the JSON-RPC 2.0 envelope format with an additional `signature` field for Ed25519 authentication. Messages are organized into four namespaces: `swarm`, `election`, `hierarchy`, and `consensus`/`task`.
